...
func TestProducerBoundary(t *testing.T) {
	valid := `{"key":"asset","invocation":{"program":"ffmpeg","args":[]},"awaitFile":"segment.ts","runTimeoutMs":1,"cacheTtlMs":1}`
	request, _, err := decodeProducerRequest(valid)
	if err != nil || request.AwaitFile != "segment.ts" {
