...
The v0.2.0 release consumes the immutable RuleGo Plugin ABI contract in
...
runtime to load each plugin and verify both `ffmpegOverIp` and
`ffmpegOverIpResponse`. Install only the architecture-matching plugin whose
checksum and ABI sidecar verify against the bundled release record.
